...
public class CompletedText extends Text {
	...
	public CompletedText (final Composite parent, int style,
					...
					case SWT.KEYPAD_CR:
						...
						if (table.getItemCount () > 0) {
							...
							if (table.getSelectionIndex () != -1)
								tempText = table.getSelection ()[0].getText ();
							else
								tempText = table.getItem (0).getText ();
		...
		addListener (SWT.Modify, new Listener () {
			...
			public void handleEvent (Event event) {
				...
				if (getText ().length () > 0 && (hints != null)
					...
					textBounds = parent.getDisplay ().map (parent, null,
							getBounds ());
		 ...
		 * 
		 * public void handleEvent (Event event) { allowTextChange = true;
		 * 
		 * setText (table.getSelection ()[0].getText ()); popupShell.setVisible
		 * (false); textSet = true; } });
		 */
	...
	public static void main (String args[]) throws ClassNotFoundException,
		...
		text.setLayoutData (new GridData (SWT.FILL, SWT.CENTER, true, true, 1,
				1));
